...
cor(povertyDataset$Firearm.related,povertyDataset$Below.Poverty.Level)
cor.test(povertyDataset$Firearm.related,povertyDataset$Below.Poverty.Level)
summary(fit.FirearmPoverty)
cor(povertyDataset$Assault..Homicide.,povertyDataset$Below.Poverty.Level)
cor.test(povertyDataset$Assault..Homicide.,povertyDataset$Below.Poverty.Level)
summary(fit.AssaultPoverty)
...
fit.Firearm = lm(Firearm.related ~ Below.Poverty.Level + Unemployment, povertyDataset) #regresija s jako koreliranim varijablama
summary(fit.Firearm)
cor(povertyDataset$Unemployment,povertyDataset$Below.Poverty.Level)

fit.Assault = lm(Assault..Homicide. ~ Below.Poverty.Level + Unemployment, povertyDataset) #regresija s jako koreliranim varijablama
summary(fit.Assault)

fit.Firearm2 = lm(Firearm.related ~ Crowded.Housing + No.High.School.Diploma, povertyDataset) #regresija s jako koreliranim varijablama
summary(fit.Firearm2)
cor(povertyDataset$Crowded.Housing,povertyDataset$No.High.School.Diploma)

fit.Assault2 = lm(Assault..Homicide. ~ Crowded.Housing + No.High.School.Diploma, povertyDataset) #regresija s jako koreliranim varijablama
summary(fit.Assault2)
```
...
cor(cbind(povertyDataset$Below.Poverty.Level,povertyDataset$Per.Capita.Income,povertyDataset$Crowded.Housing)) # korelacijski koeficijenti parova regresora

```

```{r visestruka regresija}
fit.multi = lm(Firearm.related ~ Below.Poverty.Level + Per.Capita.Income + Crowded.Housing, povertyDataset)
summary(fit.multi)

fit.multi = lm(Assault..Homicide. ~ Below.Poverty.Level + Per.Capita.Income + Crowded.Housing, povertyDataset)
summary(fit.multi)
